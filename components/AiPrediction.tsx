      ...
      <div className="wrap">
        <div className="ai-panel">
          ...
          <form className="ai-form" onSubmit={submit} onInput={() => { setFreePredictionUsed(false); setPaidPaymentId(""); }}>
            ...
            <div className="ai-two birth-time-place-row" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1fr)", gap: "12px", width: "100%" }}>
              <div className="field"><label htmlFor="ai-time">{t.birthTime}</label><input id="ai-time" name="birthTime" type="time" value={birthTime} onChange={(e) => setBirthTime(e.target.value)} step="60" required style={{ width: "100%", minWidth: 0, boxSizing: "border-box" }} /><small className="field-hint">{language === "hi" ? "सटीक घंटा और मिनट चुनें। " : language === "mr" ? "अचूक तास आणि मिनिट निवडा." : "Select the exact hour and minute."}</small></div>
