...
function readSessionToken(token: string | undefined): AdminSession | null {
  ...
  if (received.length !== expectedBuffer.length || !crypto.timingSafeEqual(expectedBuffer, received)) return null;
  try {
...
export async function getAdminSession() { const store = await cookies(); return readSessionToken(store.get(COOKIE_NAME)?.value); }
...
export async function setSessionCookie(session: AdminSession) {
  ...
  store.set(COOKIE_NAME, createSessionToken(session), { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: MAX_AGE });
}
